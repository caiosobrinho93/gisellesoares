  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-500
      ...
      <div className="container-luxe flex justify-between items-center">
        ...
        {/* Mobile Toggle */}
        <button onClick={() => setIsOpen(!isOpen)} className="lg:hidden p-3 bg-gray-50 rounded-xl text-black hover:bg-gray-100 transition-all">
          {isOpen ? <X size={24} /> : <Menu size={24} />}
        </button>
      </div>

      {/* Mobile Menu - Professional Luxe */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
